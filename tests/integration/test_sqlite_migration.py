...
# 22 run-path tables (v1) + 6 catalog tables (v2).
EXPECTED_TABLES = 28
...
def test_clean_install_creates_the_whole_schema(sqlite_path: Path) -> None:
    ...
    try:
        ...
        # v2 catalog tables (scenario/revision/snapshot persistence).
        assert {
            "catalog_profile",
            "catalog_profile_revision",
            "catalog_scenario",
            "catalog_scenario_revision",
            "catalog_snapshot",
            "catalog_scenario_run",
        } <= tables
    finally:
        connection.close()
