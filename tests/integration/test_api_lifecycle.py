...
def test_profile_revision_content_round_trips_the_payload(client: TestClient) -> None:
    """A satellite preset (SPACECRAFT profile, opaque payload) can be read back over HTTP."""
    payload = {
        "schema": "passbudget-satellite-preset-1",
        "name": "관측위성 A (가정)",
        "provenance": "가정 궤도",
        "isAssumption": True,
        "orbit": {"kind": "GP_TLE", "tle": {"line_1": "1" * 69, "line_2": "2" * 69}},
    }
    created = client.post(
        "/api/v1/profiles",
        json={
            "stable_key": "SAT-CONTENT-01",
            "kind": "SPACECRAFT",
            "name": payload["name"],
            "is_preset": True,
        },
    ...
    profile_id = created.json()["profile_id"]
    revision = client.post(
        f"/api/v1/profiles/{profile_id}/revisions", json={"label": "v1", "payload": payload}
    )
    revision_id = revision.json()["revision_id"]
    client.post(f"/api/v1/profile-revisions/{revision_id}/publish")

    content = client.get(f"/api/v1/profile-revisions/{revision_id}/content")
    assert content.status_code == 200
    body = content.json()
    assert body["profile_kind"] == "SPACECRAFT"
    assert body["payload"] == payload  # opaque payload round-trips exactly

    missing = client.get("/api/v1/profile-revisions/does-not-exist/content")
    assert missing.status_code == 404
    assert missing.json()["error"]["code"] == "PROFILE_REVISION_NOT_FOUND"
