...
@pytest.mark.parametrize(
    ...
    [
        "README.md",
        "docs/architecture/ADR-0002-synthetic-contact-provenance.md",
        "docs/architecture/ADR-0003-persistence-tiers.md",
        "docs/specs/SPEC_CONFLICT_REGISTER.md",
        "docs/specs/P0_ACCEPTANCE_MATRIX.md",
        "docs/specs/ORBIT_RELEASE_GATE.md",
    ],
)
...
@pytest.mark.parametrize(
    "document", ["docs/specs/P0_FUNCTIONAL_SPEC.md", "docs/prompts/BACKEND_KICKOFF_PROMPT.md"]
)
def test_frozen_documents_carry_the_relocation_note(document: str) -> None:
    """Their body is unedited, so the note is the only thing keeping their paths findable."""
    text = (ROOT / document).read_text(encoding="utf-8")
    assert RELOCATION_NOTE in text
    assert "db/postgresql/schema_v0_1.sql" in text
    assert "db/sqlite/schema_v1.sql" in text


def test_the_superseded_persistence_clause_is_marked() -> None:
    """ADR-0001's persistence paragraph stays, but a reader must see that it was superseded."""
    text = (ROOT / "docs/architecture/ADR-0001-runtime-and-portability.md").read_text(
        encoding="utf-8"
    )
    assert "Superseded on 2026-09-03 by [ADR-0003]" in text
    assert "PostgreSQL 16+ is the P0 authority" in text, "the original wording is preserved"
